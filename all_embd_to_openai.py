...
class EmbeddingRequest(BaseModel):
    # 输入可以是字符串、字符串列表、至于为什么要加上List[List[int]]，因为在集成maxkb的过程中发现，其调用向量模型传过来的参数中文字是经过tiktoken编码的,需要解码。
...
class OpenaiServer:
    ...
    def register_routes(self):
        ...
        self.app.add_api_route("/v1/embeddings", self.get_embeddings, methods=["POST"])
        ...
        if isinstance(request.input, List):
            if isinstance(request.input[0], str):
                ...
                for i, item in enumerate(embedding):
                    tmp = {"object": "embedding", "embedding": item, "index": i}
                    data.append(tmp)
            ...
            tmp = {"object": "embedding", "embedding": embedding, "index": 0}
            data.append(tmp)
...
def entrypoint(model_dir, host: Optional[str] = None, port: int = 8000):
