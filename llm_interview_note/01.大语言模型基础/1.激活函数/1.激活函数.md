...
梯度消失和梯度爆炸的根本原因在于**反向传播时的链式法则（Chain Rule）与网络深度（或时间步数）的指数级叠加效应**。具体原因可分为以下 4 个核心维度：

2.1 链式法则与深层矩阵连乘（核心结构原因）

在 $N$ 层神经网络的反向传播中，第 $l$ 层的损失函数梯度计算公式为：

$$\frac{\partial L}{\partial W_l} = \left( \prod_{k=l+1}^{N} W_k^T \cdot \text{diag}(f'(z_k)) \right) \cdot \frac{\partial L}{\partial z_N}$$

diag 是线性代数中的对角矩阵（Diagonal Matrix）符号。假设第 $k$ 层的神经元输入向量 $z_k$ 有 $m$ 个维度，即 $z_k = [z_{k,1}, z_{k,2}, \dots, z_{k,m}]^T$。乘以对角矩阵等导于将前面传过来的梯度按元素依次乘以该维度的激活函数导数 $f'(z_{k,i})$。

* **梯度消失**：当权重矩阵 $W_k$ 的特征值（或奇异值）普遍小于 1.0，且**激活函数导数 $f'(z_k) \le 1.0$ 时**，连续相乘 $N-l$ 次会导致梯度呈**指数级衰减**趋近于 0。浅层网络的权重无法获得有效的更新信号。
* **梯度爆炸**：当权重矩阵 $W_k$ 的主特征值大于 1.0 时，连续相乘会导致梯度呈**指数级增长**，造成数值溢出（出现 `NaN` 或 `Inf`），模型权重剧烈震荡无法收敛。

2.2 饱和激活函数的导数压缩（函数选择原因）

使用带有“饱和区”的非线性激活函数会加速梯度的衰减：

* **`Sigmoid` 函数**：其导数公式为 $f'(x) = f(x)(1 - f(x))$，最大值仅为 **0.25**。每经过一层，梯度至少被压缩为原来的 1/4。只需 5~6 层的连乘，梯度就会衰减到 $10^{-4}$ 级别。
* **`Tanh` 函数**：其导数最大值为 **1.0**，但在输入值较大或较小时（$\vert{}x\vert{} \gg 0$），函数进入饱和区，导数 $f'(x) \to 0$，导致传回的梯度直接中断。

2.3 不恰当的参数初始化（数值分布原因）

网络权重的初始方差与分布直接决定了初始阶段雅可比矩阵的谱半径：

* **初始化过小**：权重矩阵的奇异值均小于 1，初始训练阶段即触发梯度消失。
* **初始化过大**：权重矩阵的奇异值显著大于 1，前向传播时激活值爆表，反向传播时瞬间触发梯度爆炸。

2.4 环神经网络（RNN）的时间序列展开

RNN 在不同时间步共享同一个权重矩阵 $W_{hh}$。在长序列（时间步 $T$ 较长）的反向传播（BPTT）中，梯度的计算包含 $W_{hh}^T$ 的 $T$ 次幂乘：


$$\frac{\partial h_T}{\partial h_1} = \prod_{t=2}^{T} W_{hh}^T \cdot \text{diag}(f'(z_t))$$

* 若 $W_{hh}$ 的最大特征值 $\lambda > 1$，梯度随时间步 $T$ 呈 $\lambda^T$ 指数爆炸，无法捕获长距离上下文。
* 若 $\lambda < 1$，梯度随时间步 $T$ 呈指数衰减，引发长距离依赖失效（Long-Term Dependency Issue）。
...
1. 饱和激活函数的导数压缩导致只需要使用像 ReLU 这种激活函数就可以解决；
2. 链式法则与深层矩阵连乘导致没有能够完全解决的方法，目前有一些方法可以很大程度上进行缓解该问题，比如：对梯度做截断解决梯度爆炸问题、残差连接、normalize。由于使用了残差连接和 normalize 之后梯度消失和梯度爆炸已经极少出现了，所以目前可以认为该问题已经解决了。
...
### 6.GeLU(Gaussian Error Linear Units, 高斯误差线性单元)
...
符号定义：输入值用 $x$ 表示，$ϕ(⋅)$表示下图8中的**正态分布的累积分布函数**，$f(⋅)$表示新设计的函数。
...
其中$x$表示实际的输入值，$X$表示随机变量。到此就得到了GELU的常见形式，公式如下所示，其中 $X<x$ 表示随机值小于 x 的面积：
...
SwiGLU(Swish Gated Linear Unit，Swish门控线性单元)的形式为

$$
\text{SwiGLU}(x, W_g, W_v, b_g, b_v) = \text{Swish}_{\beta}(x W_g + b_g) \odot (x W_v + b_v)
$$

这里有一个新符号 $g(x)$ 表示的是向量 $x$ 经过一层MLP或者卷积， $⊗$ 表示两个向量逐元素相乘， $σ$ 表示 sigmoid 函数。
