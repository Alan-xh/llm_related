...
* $Q(s, a)$：在状态 $s$ 下采取动作 $a$ 所能获得的预期累积回报（价值）。
* $s$：环境当前所处的状态（State）。
* $a$：智能体采取的动作（Action）。
* $f$：用于近似 $Q$ 值的拟合函数（如神经网络）。
* $w$：拟合函数 $f$ 的可学习权重/参数（Weight）。

在PG算法中，因为策略是一个概率，不能直接用来迭代，所以采取类似的思路，将其转化为函数形式，如下式所示，这时的目的则是**使用带有 $\theta$ 参数的函数对策略进行近似，通过更新参数 $\theta$，逼近最优策略**。

$$\pi(a \mid s) \approx \pi_\theta(s, a) = P(a \mid s, \theta)$$

* $\pi(a \mid s)$：在状态 $s$ 下选择动作 $a$ 的真实/最优概率分布策略。
* $\pi_\theta(s, a)$ 或 $P(a \mid s, \theta)$：以 $\theta$ 为参数的模型在状态 $s$ 下输出动作 $a$ 的条件概率分布（参数化策略）。
* $\theta$：策略函数的参数向量（如神经网络的权重与偏置）。
