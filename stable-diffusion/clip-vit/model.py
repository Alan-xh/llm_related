...
class Bottleneck(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1):
        '''
        inplanes: 等价于 in_channel, 在 resnet 中比较常见
        planes: 等价于 out_channel, 在 resnet 中比较常见
        stride: 步幅, 池化
        '''
        super().__init__()

        # 所有卷积层的步长均为 1。当步长 > 1 时，在第二次卷积之后执行平均池化操作。
        self.conv1 = nn.Conv2d(inplanes, planes, kernel_size=1, bias=False) # channel: inplanes -> planes
        self.bn1 = nn.BatchNorm2d(planes) # planes 0 维度上每个值 (x - 均值)/标准差 

        self.conv2 = nn.Conv2d(planes, planes, kernel_size=3, padding=1, bias=False)
        self.bn2 = nn.BatchNorm2d(planes)

        self.avgpool = nn.AvgPool2d(stride) if stride > 1 else nn.Identity() # AvgPool2d 取平均值池化，Identity 占位符，表示不改变任何数值和训练参数

        self.conv3 = nn.Conv2d(planes, planes * self.expansion, 1, bias=False)
        self.bn3 = nn.BatchNorm2d(planes * self.expansion)

        self.relu = nn.ReLU(inplace=True) # 激活函数，覆盖张量 y = x if x > 0 else 0
        self.downsample = None
        self.stride = stride

        if stride > 1 or inplanes != planes * Bottleneck.expansion:
            # 下采样层前面接一个平均池化层，处理需要降采样或维度匹配的恒等映射，-1，0，1表示名称,计算依次进行
...
class ModifiedResNet(nn.Module):
    ...
    - 现在有 3 个“茎”卷积层，而不是 1 个，并且使用平均池化层代替了最大池化层。

    - 执行抗锯齿步长卷积，其中步长大于 1 的卷积层前面会添加一个平均池化层。

    - 最后一个池化层是 QKV 注意力层，而不是平均池化层。
    """
