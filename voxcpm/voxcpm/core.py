    ...
    def _generate(
        ...
        try:
            ...
            generate_result = self.tts_model._generate_with_prompt_cache(
                target_text=text,  # 要转换为语音的文本
                prompt_cache=fixed_prompt_cache,  # 由 build_prompt_cache 构建的缓存（可以为 None）
                min_len=2,  # 最小音频长度，避免音频太短
                max_len=max_length,  # 最大音频长度
                inference_timesteps=inference_timesteps,  # 扩散采样步骤数
                cfg_value=cfg_value,  # 无分类器指导值
                retry_badcase=retry_badcase,  # 是否对不良案件进行重审
                retry_badcase_max_times=retry_badcase_max_times,  # 最大重试次数
                retry_badcase_ratio_threshold=retry_badcase_ratio_threshold,  # 音频与文本比例的阈值
