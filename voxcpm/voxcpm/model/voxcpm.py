...
class VoxCPMEncoderConfig(BaseModel):
    ...
    ffn_dim: int = 4096  # FFN 层维度
    num_heads: int = 16  # 多头注意力头数
    num_layers: int = 4  # 多头注意力层数
    kv_channels: int = None  # 键值通道数量
...
class VoxCPMConfig(BaseModel):
    lm_config: MiniCPM4Config  # 基础模型配置
    patch_size: int = 2  # 推理批次
    feat_dim: int = 64  # 音频特征维度
    residual_lm_num_layers: int = 6  #  residual lm 的层数
    scalar_quantization_latent_dim: int = 256  # 矢量量化维度
    scalar_quantization_scale: int = 9  # 矢量量化的缩放

    encoder_config: VoxCPMEncoderConfig  # 语义编码器配置
    dit_config: VoxCPMDitConfig  # 语义解码器配置

    max_length: int = 4096  # 最大文本长度
    device: str = "cuda"
    dtype: str = "bfloat16"
...
class VoxCPMModel(nn.Module):
    def __init__(
        ...
        super().__init__()
        self.config = config  # 模型配置对象：包含模型所需的各种超参数和子模块配置。
        self.feat_dim = (
            config.feat_dim
        )  # 特征维度：声学特征（如梅尔频谱或 VAE 潜在码）的维度。
        self.patch_size = (
            config.patch_size
        )  # 特征块大小：在局部特征处理中，特征被切分的块（patch）的大小。
        self.device = config.device  # 设备：模型运行的设备，如 "cuda" 或 "cpu"。
        ...
        self.base_lm = MiniCPMModel(
            config.lm_config
        )  # 基础语言模型 (MiniCPMModel)：负责处理文本输入并捕捉文本语义信息的语言模型。
        self.base_lm.setup_cache(
            1, config.max_length, self.device, get_dtype(config.dtype)
        )

        self.text_tokenizer = mask_multichar_chinese_tokens(
            tokenizer
        )  # 文本分词器 (LlamaTokenizerFast)：用于将输入文本转换为 token ID，并经过特殊处理（如中文多字符 token 掩码）。
        self.audio_start_token = (
            101  # 音频起始 token ID：用于标识音频序列开始的特殊 token 的 ID。
        )
        self.audio_end_token = (
            102  # 音频结束 token ID：用于标识音频序列结束的特殊 token 的 ID。
        )
        ...
        self.residual_lm = MiniCPMModel(
            residual_lm_config
        )  # 残差声学语言模型 (MiniCPMModel)：一个较小的语言模型，可能用于建模声学特征中的残差或更精细的声学模式。
        ...
        self.feat_encoder = VoxCPMLocEnc(
            encoder_config, input_dim=config.feat_dim
        )  # 局部编码器 (VoxCPMLocEnc)：一个 Transformer 编码器，用于从声学特征中提取局部上下文信息。
        ...
        )  # 局部 DiT (Diffusion Transformer) 解码器 (UnifiedCFM)：基于 Diffusion Transformer 的解码器，用于高保真地生成声学特征。
        ...
        )  # 标量量化层 (ScalarQuantizationLayer)：用于将 LM 的隐藏状态进行量化，可能用于离散化或信息压缩。
        self.enc_to_lm_proj = nn.Linear(
            config.encoder_config.hidden_dim, config.lm_config.hidden_size
        )  # 编码器到 LM 投影层 (nn.Linear)：将局部编码器输出的特征投影到基础 LM 的隐藏维度。
        self.lm_to_dit_proj = nn.Linear(
            config.lm_config.hidden_size, config.dit_config.hidden_dim
        )  # LM 到 DiT 投影层 (nn.Linear)：将基础 LM 的隐藏状态投影到 DiT 解码器的隐藏维度，作为生成条件。
        self.res_to_dit_proj = nn.Linear(
            config.lm_config.hidden_size, config.dit_config.hidden_dim
        )  # 残差 LM 到 DiT 投影层 (nn.Linear)：将残差 LM 的隐藏状态投影到 DiT 解码器的隐藏维度，可能作为残差条件的输入。

        # Stop Predictor
        self.stop_proj = nn.Linear(
            config.lm_config.hidden_size, config.lm_config.hidden_size
        )  # 停止预测投影层 (nn.Linear)：用于将 LM 隐藏状态投影到更小的维度，作为停止预测头的前置层。
        self.stop_actn = (
            nn.SiLU()
        )  # 停止预测激活函数 (nn.SiLU)：用于停止预测层的激活函数。
        self.stop_head = nn.Linear(
            config.lm_config.hidden_size, 2, bias=False
        )  # 停止预测头 (nn.Linear)：一个线性分类器，用于预测音频序列是否应该停止。

        # Audio VAE
        self.audio_vae = audio_vae  # 音频 VAE (AudioVAE)：用于将原始音频编码成潜在特征，并在生成时解码回音频。
        self.chunk_size = (
            audio_vae.chunk_size
        )  # 块大小：Audio VAE 处理音频数据时的一个时间/样本块的大小。
        self.sample_rate = (
            audio_vae.sample_rate
        )  # 采样率：Audio VAE 处理的音频的采样率。
    ...
    def optimize(self, disable: bool = False):
        try:
            ...
            print(f"Error: {e}")
            print(
                "Warning: VoxCPMModel can not be optimized by torch.compile, using original forward_step functions"
            )
    ...
    def _generate(
        ...
        if not streaming:
            ...
            decode_audio = decode_audio[
                ..., 640:-640
            ]  # trick: trim the start and end of the audio
            yield decode_audio
    ...
    def _inference(
        ...
        Tuple[torch.Tensor, Union[torch.Tensor, List[torch.Tensor]]], None, None
    ]:
