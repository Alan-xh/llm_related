...
def main() -> None:
    parser = argparse.ArgumentParser(description="Train the tiny SDXL teaching model.")
    parser.add_argument("--steps", type=int, default=5)
    parser.add_argument("--batch-size", type=int, default=2)
    parser.add_argument("--image-size", type=int, default=32)
    parser.add_argument("--lr", type=float, default=1e-4)
    parser.add_argument("--device", default="cpu")
    parser.add_argument("--checkpoint", default="sdxl_tiny.pt")
    args = parser.parse_args()

    device = torch.device(args.device)
    torch.manual_seed(0)
    model = build_model().to(device)
    optimizer = torch.optim.AdamW(model.base.parameters(), lr=args.lr)
    model.train()
    ...
    torch.save({"base": model.base.state_dict(), "refiner": model.refiner.state_dict()}, args.checkpoint)
