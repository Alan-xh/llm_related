...
def build_toy_videos(
    ...
    for index in range(batch_size):
        ...
        for frame in range(frames):
            ...
            mask = (((xx - phase * 0.5 + 0.2) ** 2 + yy**2) < 0.35).float()
            values.append(
                torch.stack((mask * 0.1, mask * 0.8, mask * 0.2), dim=0) * 2.0 - 1.0
            )
        videos.append(torch.stack(values, dim=1))
        prompts.append("a green object moving right")
    return torch.stack(videos), prompts


def main() -> None:
    parser = argparse.ArgumentParser(
        description="Train the tiny Wan2.2 TI2V MoE model."
    )
    ...
    parser.add_argument("--checkpoint", default="wan22_tiny.pt")
    ...
    for step in range(args.steps):
        ...
        loss = model.training_loss(videos, prompts, videos[:, :, 0])
