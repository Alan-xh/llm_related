"""Kimi K2 教学模型的 CPU smoke-training 入口。

任务定义:
    使用重复文本构造因果语言建模 batch，执行若干步 AdamW 更新并保存
    教学 checkpoint。
...
def main() -> None:
    """解析命令行参数并运行 Kimi K2 的最小训练循环。"""
    parser = argparse.ArgumentParser(description="训练 Kimi K2 风格的最小 MLA/MoE 模型。")
    ...
    parser.add_argument(
        "--text",
        default="Kimi K2 uses latent attention and sparse experts for agentic language modeling. ",
    )
    parser.add_argument("--device", default="cpu")
    parser.add_argument("--checkpoint", default="kimi_k2_tiny.pt")
